    ...
    def post(self, request, format=None):
        ...
        if "dbname" in parser_dict:
            ...
            if db_type == "WebScans":
                ...
                return Response({"error": "SAST uploads are disabled"}, status=status.HTTP_404_NOT_FOUND)
                ...
                return Response({"error": "Cloud uploads are disabled"}, status=status.HTTP_404_NOT_FOUND)
...
class AuditLogList(APIView):
    ...
    def get(self, request):
        ...
        if org_id:
            qs = qs.filter(user__organization_id=org_id)
        ...
        results = qs.order_by("-created_at")[start:start + page_size]

        from archeryapi.serializers import AuditLogSerializer
...
class NotificationList(APIView):
    ...
    def get(self, request):
        ...
        total = qs.count()
        start = (page - 1) * page_size
        results = qs.order_by("-timestamp")[start:start + page_size]

        from archeryapi.serializers import NotificationSerializer
...
class NotificationPreferences(APIView):
    ...
    def get(self, request):
        from archeryapi.serializers import NotificationPreferencesSerializer
        u = request.user
        serializer = NotificationPreferencesSerializer({
            "notify_email": u.notify_email,
            "notify_in_app": u.notify_in_app,
            "notify_on_scan_start": u.notify_on_scan_start,
            "notify_on_scan_complete": u.notify_on_scan_complete,
            "notify_on_scan_fail": u.notify_on_scan_fail,
    ...
    def put(self, request):
        ...
        if not serializer.is_valid():
            return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
        u = request.user
        for key in ("notify_email", "notify_in_app", "notify_on_scan_start",
                     "notify_on_scan_complete", "notify_on_scan_fail", "notify_critical_only"):
            if key in serializer.validated_data:
                setattr(u, key, serializer.validated_data[key])
        u.save(update_fields=list(serializer.validated_data.keys()))
        return Response({"message": "Preferences updated"})


class ReportDownloadAPI(APIView):
    permission_classes = [IsAuthenticated | permissions.VerifyAPIKey]

    def post(self, request):
        from archeryapi.serializers import ReportDownloadSerializer
        serializer = ReportDownloadSerializer(data=request.data)
        if not serializer.is_valid():
            return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)

        project_ids = serializer.validated_data.get("project_id") or []
        scan_types = serializer.validated_data.get("scan_types") or []
        fmt = serializer.validated_data.get("format", "pdf")
        severities = serializer.validated_data.get("severity") or []
        ...
        if not project_ids:
            params.append(("project_id", "__all__"))
        for st in scan_types:
            params.append(("scan_types", st))
        for sev in severities:
            params.append(("severity", sev))
        for sec in sections:
            params.append(("sections", sec))

...
def nikto_html_parser(data, project_id, scan_id, request=None):
    ...
    # --- Advanced severity mapping rules ---
    BASE_SCORES = {"Info": 0, "Low": 3, "Medium": 7, "High": 15, "Critical": 40}
    ...
    RX = {
        ...
        # 2
        "phpinfo": re.compile(r"(<title>phpinfo\(|<h1>PHP Version|Fatal error on line|Stacktrace)", re.I),
        # 3
        "dir_index": re.compile(r"Index of /", re.I),
        # 4
        ...
        # 18
        "methods": re.compile(r"Allow:\s*(?:.*\b)?(PUT|DELETE|TRACE)\b", re.I),
        # 19
    ...
    def _eval_rules(text):
        ...
        if RX["dir_index"].search(text):
            # escalate to High if sensitive files mentioned
        ...
        if RX["exposed_src"].search(text):
            # escalate to Critical if secrets too
            if RX["secrets"].search(text):
                found.append(add("Critical", "High"))
            else:
                found.append(add("High", "High"))
        ...
        if RX["headers"].search(text):
            # Assume missing when Nikto reports – keep Low; bump if multiple
            # We'll count occurrences
            m = RX["headers"].findall(text)
            if m and len(m) >= 2:
                found.append(add("Medium", "Medium"))
            else:
                found.append(add("Low", "Medium"))
        ...
        if RX["archives"].search(text):
            # Escalate to Critical if description hints credentials
            if RX["secrets"].search(text):
                found.append(add("Critical", "High"))
            else:
                found.append(add("High", "High"))
        ...
        score = sum(found)
        if len(found) >= 2:
            score *= 1.25  # correlation multiplier
    ...
    for link in soup.find_all(class_="dataTable"):
        ...
        risk = _eval_rules(discription + "\n" + (uri or ""))
        vul_col = _sev_color(risk)

...
class burp_scans(object):
        ...
        while int(scan_status) < 100:
            if (time.time() - start_ts) > 3600:
                ...
                WebScansDb.objects.filter(
                    ...
                    scanner="Burp",
                    organization=self.user.organization,
                ).update(scan_status="100",)
                try:
                    notify.send(self.user, recipient=self.user, verb="Burp scan timed out after 1 hour")
                except Exception:
                    pass
                break
            scan_info = bi.scan_info(burp_scan_id)
    ...
    def burp_scan_data(self, scan_data):
        ...
        for data in scan_data:
            ...
            match_dup = (
                WebScanResultsDb.objects.filter(
                    dup_hash=duplicate_hash, organization=self.user.organization
                )
                .values("dup_hash")
                .distinct()
            )
        ...
        burp_all_vul = (
            WebScanResultsDb.objects.filter(
                scan_id=self.scan_id, organization=self.user.organization
            )
        ...
        WebScansDb.objects.filter(
            scan_id=self.scan_id, scanner="Burp", organization=self.user.organization
        ).update(
            total_vul=total_vul,

...
# Helpers: per-scan ZAP log path + write
...
def _delete_web_scan_artifacts(scan_id, organization, created_by=None):
    """
    Remove all persisted data and per-scan files for a web scan.
    """
    # Primary findings/results
    WebScanResultsDb.objects.filter(scan_id=scan_id, organization=organization).delete()

    # Legacy/auxiliary tables linked to the scan id
    try:
        from tools.models import NiktoResultDb, NiktoVulnDb

        nikto_filter = {"scan_id": scan_id, "organization": organization}
        if created_by:
            nikto_filter["created_by"] = created_by
        NiktoResultDb.objects.filter(**nikto_filter).delete()
        NiktoVulnDb.objects.filter(**nikto_filter).delete()
    except Exception:
        pass

    try:
        from webscanners.models import web_scan_db

        legacy_filter = {"scan_id": scan_id, "organization": organization}
        if created_by:
            legacy_filter["created_by"] = created_by
        web_scan_db.objects.filter(**legacy_filter).delete()
    except Exception:
        pass

    # Best-effort filesystem cleanup (logs, HTML artifacts)
    nikto_log_dir = getattr(settings, "NIKTO_LOG_DIR", os.path.join(os.getcwd(), "logs", "nikto"))
    nikto_res_dir = getattr(settings, "NIKTO_RESULT_DIR", os.path.join(os.getcwd(), "nikto_result"))
    _safe_remove(os.path.join(nikto_log_dir, f"{scan_id}.log"))
    _safe_remove(os.path.join(nikto_res_dir, f"{scan_id}.html"))
    _safe_remove(_zap_scan_log_path(scan_id))
...
class WebScanList(APIView):
    ...
    def get(self, request):
        ...
        # Scope by organization; non-admins only see their own scans. Admin sees their own only.
...
class AdminLogs(APIView):
    ...
    def get(self, request):
        return render(request, "webscanners/admin_logs.html", {})
...
class WebScanVulnInfo(APIView):
    ...
    def get(self, request, uu_id=None):
        ...
        if uu_id is None:
            ...
            if not is_admin:
                scan_qs = scan_qs.filter(organization=request.user.organization, created_by=request.user)
            if not scan_qs.exists():
                return HttpResponseRedirect(reverse("webscanners:list_scans"))
            vuln_filter = {"title": name, "scan_id": scan_id}
            if not is_admin:
                vuln_filter["organization"] = request.user.organization
            vuln_data = WebScanResultsDb.objects.filter(**vuln_filter)
        else:
            try:
                is_admin = getattr(request.user, 'is_superuser', False) or str(getattr(request.user, 'role', '')) in ('Admin', 'Organization Admin')
                # Validate access to the scan id provided in the path
                scan_qs = WebScansDb.objects.filter(scan_id=uu_id)
                if not is_admin:
                    scan_qs = scan_qs.filter(organization=request.user.organization, created_by=request.user)
                if not scan_qs.exists():
                    return HttpResponseRedirect(reverse("webscanners:list_scans"))
        ...
        if request.path[:4] == "/api":
            ...
            return render(
                ...
                {"vuln_data": vuln_data, "jira_url": jira_url, "mitre_facets": mitre_facets},
            )
...
class WebScanDetails(APIView):
    ...
    def get(self, request):
        ...
        try:
            ...
            is_admin = (role in ("Admin", "Organization Admin")) or getattr(request.user, "is_superuser", False)
        except Exception:
            is_admin = False
        ...
        vul_dat = list(vul_qs.order_by("vuln_id"))

        # Extract CVEs (if present) from reference/description/title for display
        def _extract_cves(*chunks):
            found = []
            for chunk in chunks:
                if not chunk:
                    continue
                found.extend(re.findall(r"CVE-\\d{4}-\\d{4,7}", str(chunk), flags=re.IGNORECASE))
            seen = set()
            result = []
            for c in found:
                uc = c.upper()
...
class WebScanDelete(APIView):
    ...
    def post(self, request):
        ...
        for i in range(0, split_length):
            ...
            if deleted_count:
                _delete_web_scan_artifacts(scan_id, request.user.organization)
        return HttpResponseRedirect(reverse("webscanners:list_scans"))
...
class WebScanVulnList(APIView):
    ...
    def get(self, request):
        ...
        try:
            ...
            available_phases = []
        all_vuln = base_vuln
        phase = request.GET.get('phase') or "All"
        if phase and phase != "All" and phase in set(available_phases):
...
class WebScanRecent(APIView):
    ...
    def get(self, request):
        ...
        # Build the base queryset and apply any role filters BEFORE slicing.
        # Django forbids filtering a QuerySet once a slice has been taken
        # (it would otherwise execute the query immediately and return a list).
        ...
        if not is_admin:
            base_qs = base_qs.filter(created_by=request.user)
        # Only now take the slice for performance
        base_qs = base_qs[:100]
...
class ZapLog(APIView):
    ...
    def get(self, request):
        ...
        # Optional: pull live alerts (Admin-only)
        alerts = []
        alert_error = None
        try:
            if role == "Admin" or getattr(request.user, "is_superuser", False):
                zap_cfg = ZapSettingsDb.objects.filter(enabled=True).first()
                if zap_cfg and zap_cfg.api_key and zap_cfg.host and zap_cfg.port:
                    base = f"http://{zap_cfg.host}:{zap_cfg.port}"
                    params = {"apikey": zap_cfg.api_key, "start": 0, "count": 20}
                    resp = requests.get(f"{base}/JSON/alert/view/alerts/", params=params, timeout=5)
                    if resp.ok:
                        data = resp.json() or {}
                        alerts = data.get("alerts") or []
                    else:
                        alert_error = f"ZAP API error {resp.status_code}"
        except Exception as e:
            alert_error = str(e)
        ...
        return render(
            request,
            "webscanners/zap_log.html",
            {"has_log": True, "initial": combined, "alerts": alerts, "alert_error": alert_error},
        )
...
class WebScannerLog(APIView):
    ...
    def get(self, request):
        ...
        try:
            ...
            max_kb = 200
        log_path = _zap_scan_log_path(scan_id)
        if not os.path.exists(log_path):
            if raw:
                return HttpResponse("", status=202, content_type="text/plain")
            return render(request, "webscanners/zap_scan_log.html", {"scan_id": scan_id, "has_log": False, "initial": ""})
        try:
            if full:
            ...
            return render(request, "webscanners/zap_scan_log.html", {"scan_id": scan_id, "has_log": True, "initial": f"Failed to read log: {e}"})
        if raw:
            return HttpResponse(content, content_type="text/plain")
        return render(request, "webscanners/zap_scan_log.html", {"scan_id": scan_id, "has_log": True, "initial": content})

class ZapScanLog(WebScannerLog):
    pass
    ...
    def post(self, request):
        ...
        try:
            if scanner == "zap":
                ...
                # Pre-touch the log so the UI can render immediately
                try:
                    from django.conf import settings as _settings
                    _log_dir = getattr(_settings, 'NIKTO_LOG_DIR', os.path.join(os.getcwd(), 'logs', 'nikto'))
                    os.makedirs(_log_dir, exist_ok=True)
                    _log_path = os.path.join(_log_dir, f"{scan_id}.log")
                    with open(_log_path, 'a', encoding='utf-8', errors='ignore') as _lf:
                        _lf.write(f"[archerysec] Queued Nikto scan for {target_url} (scan_id={scan_id})\n")
                except Exception:
                    pass

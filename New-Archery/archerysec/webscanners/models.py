...
class WebScansDb(models.Model):
    ...
    # Optional human-readable reason when a scan effectively failed
    ...
    # Human-readable scan type for this run (e.g. "ZAP Active + Spider")
    scan_type = models.TextField(blank=True, null=True)
    # ZAP active scan id (for precise stop); nullable and optional
    zap_ascan_id = models.TextField(blank=True, null=True)
...
class task_schedule_db(models.Model):
    ...
    schedule_time_utc = models.DateTimeField(blank=True, null=True)
    last_run_at = models.DateTimeField(blank=True, null=True)
    project = models.ForeignKey(
        "projects.ProjectDb", on_delete=models.CASCADE, null=True
    )
    scanner = models.TextField(blank=True, null=True)
    # Optional human readable description of what this schedule will run
    scan_type = models.TextField(blank=True, null=True)
    # Serialized options/flags required to launch the scan later
    scan_config = models.JSONField(blank=True, null=True, default=dict)
